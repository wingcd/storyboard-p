...
export class TextInput extends TextField  implements ITextField{ 
    static TYPE = "textinput";
    static SERIALIZABLE_FIELDS: ISerializeFields = Object.assign(
        {} as ISerializeFields,
        clone(TextField.SERIALIZABLE_FIELDS),
        {            
    ...
    {      
        let fields = TextInput.SERIALIZABLE_FIELDS;  
    ...
    private _initEditor() {
        ...
        if(textfield && this._editable) {
            ...
            let align = 'left';            
            let width = this.width - TextField.GUTTER_X * 2;
            let paddingLeft = TextField.GUTTER_X;
            ...
            let paddingTop = 0;
            let height = Math.min(this.fontSize + TextField.GUTTER_Y * 2, this.height);
            switch(this.verticalAlign) {
                case EVertAlignType.Top:
                    paddingTop = TextField.GUTTER_Y;
                    break;
                case EVertAlignType.Middle:
                    paddingTop = (this.height - height) / 2;
                    break;
                case EVertAlignType.Bottom:
                    paddingTop = (this.height - height) - TextField.GUTTER_Y;
                    break;
            }

            TextInput.isTyping = true;            
            this._editor.open({
                x:0 ,y:0,
